...
public class SmokeParticleSystem {
	...
	public SmokeParticleSystem(float pps, float speed, float gravityComplient, float lifeLength, float scale) {
		this.pps = pps;
		this.speed = speed;
	...
	private void emitParticle(Vector3f centre, float spread, Loader loader, Chunk terrain) {
		...
		new SmokeParticle(new Vector3f(posX, centre.y, posZ), velocity, gravityComplient, lifeLength, speed, 0, scale * ((float) (Math.random() + 1f)), false, loader);
	}
