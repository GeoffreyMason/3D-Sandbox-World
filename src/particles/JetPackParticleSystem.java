...
public class JetPackParticleSystem {
	...
	public JetPackParticleSystem(float pps, float speed, float gravityComplient, float lifeLength, float scale) {
		this.pps = pps;
		this.speed = speed;
	...
	private void emitParticle(Vector3f centre, float spread, Loader loader, Chunk terrain) {
		float dirX = ((float) Math.random() * 2f - 1f) * 0.1f;
		float dirZ = ((float) Math.random() * 2f - 1f) * 0.1f;
		...
		new FireParticle(new Vector3f(posX, centre.y, posZ), velocity, gravityComplient, lifeLength, speed, 0, scale * ((float) (Math.random() + 1f)), false, loader);
	}
