...
public class ParticleRenderer {
	...
	// -0.5f}; // diamond
	//private static final float[] VERTICES = { 0, 0.8f, -0.5f, 0, 0.5f, 0, 0, -0.8f }; // stretched
																						// diamond
	...
	protected void render(List<Particle> particles, Camera camera, Vector4f clipPlane) {
		Matrix4f viewMatrix = Maths.createViewMatrix(camera);
		for (Particle particle : particles) {
			prepare(particle, clipPlane);
	...
	private void updateModelViewMatrix(Vector3f position, float rotation, float scale, Matrix4f viewMatrix) {
		...
		shader.loadModelViewMatrix(modelMatrix, viewMatrix, modelViewMatrix);
	}

	private void prepare(Particle particle, Vector4f clipPlane) {
		shader.start();
